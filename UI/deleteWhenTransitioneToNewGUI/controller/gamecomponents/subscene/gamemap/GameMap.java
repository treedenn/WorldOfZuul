...
import UI.deleteWhenTransitioneToNewGUI.controller.gamecomponents.Planet;
import UI.deleteWhenTransitioneToNewGUI.controller.gamecomponents.Star;
...
public class GameMap extends InterfaceElement implements IMap {
    ...
    public void renderPlanets(Map<String, ? extends IPlanet> planets){
        for (IPlanet planet : planets.values()) {
            Planet newUIPlanet = new UI.deleteWhenTransitioneToNewGUI.controller.gamecomponents.Planet(planet, new Image(planet.getMap2D().toURI().toString().replace("\\", "/")));
